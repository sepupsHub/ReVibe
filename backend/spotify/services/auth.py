...
from .client import spotify_delete, spotify_get, spotify_post
...
def spotify_post_or_raise(access_token, path, payload=None):
    try:
        return spotify_post(access_token, path, payload=payload)
    except requests.HTTPError as exc:
        response = exc.response
        if response is None:
            raise SpotifyAPIError("Spotify request failed") from exc
        ...
        raise SpotifyAPIError("Failed to send Spotify data") from exc


def spotify_delete_or_raise(access_token, path, payload=None):
    try:
        return spotify_delete(access_token, path, payload=payload)
    except requests.HTTPError as exc:
        response = exc.response
        if response is None:
            raise SpotifyAPIError("Spotify request failed") from exc
